...
function ResultLoader(props: {
  ...
  if (apiCalled) {
    return (
      <div className="loadingText jump">
        <div className="loader "></div>
        <div className="loadingText jump">Loading...</div>
      </div>
    );
  }
  if ((submitPressed && !handleData && !apiCalled) || handleData.length === 0) {
    return (
      <div className="noResultsText">No results found, please try again.</div>
    );
  } else {
    return <Results handleData={handleData} />;
  }
}
